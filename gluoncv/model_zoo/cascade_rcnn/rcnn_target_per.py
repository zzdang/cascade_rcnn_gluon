...
from ...nn.coder import MultiClassEncoder, NormalizedBoxCenterEncoder, NormalizedBoxCenterEncoder_Cascade
...
class RCNNTargetGenerator(gluon.Block):
    ...
    def __init__(self, num_class, means=(0., 0., 0., 0.), stds=(.1, .1, .2, .2)):
        ...
        self._box_encoder_ = NormalizedBoxCenterEncoder(
             means=means, stds=stds)
        # self._box_encoder = NormalizedBoxCenterEncoder_Cascade(
    ...
    def forward(self, roi, samples, matches, gt_label, gt_box):
        ...
        with autograd.pause():
            ...
            box_target, box_mask = self._box_encoder_(samples, matches, roi, gt_box)
            # print("~~~~~~")
            # print(box_target_.shape)
