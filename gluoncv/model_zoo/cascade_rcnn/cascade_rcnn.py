...
class CascadeRCNN(RCNN2):
    ...
    def hybrid_forward(self, F, x, gt_box=None):
        ...
        roi_2nd = self.decode_bbox(source_bbox=rpn_box, \
            encoded_bbox=F.squeeze(box_pred.transpose((0, 2, 1, 3)), axis=1), stds=(.05, .05, .1, .1))
        # roi_2nd_score = 
        if autograd.is_training():
        ...
        roi_3rd = self.decode_bbox(source_bbox=roi_2nd, \
            encoded_bbox=F.squeeze(box_pred_2nd.transpose((0, 2, 1, 3)), axis=1), stds=(.033, .033, .067, .067))
        if autograd.is_training():
